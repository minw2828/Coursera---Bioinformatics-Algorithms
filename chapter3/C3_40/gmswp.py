...
#              GREEDYMOTIFSEARCH(Dna,k,t) with pseudocounts. If at any step you 
#              find more than one Profile-most probable k-mer in a given string,
...
#
# Note:
# As illustrated above, Pr(TCGTGGATTTCC | Profile) is equal to 0 because TCGTGGATTTCC 
# has a symbol corresponding to zero probability at just a single position, and so the 
# entire string is assigned a zero probability, even though TCGTGGATTTCC differs from 
# the consensus string only at the fourth position. As a result, TCGTGGATTTCC receives 
# the same probability as AAATCTTGGAA, which is very different from the consensus string. 
# In order to avoid columns that contain zeroes in profiles, bioinformaticians often 
...
def read_file(input_file):
    '''
    >>> k,t,Dna = read_file('test.gmswp.txt')
    >>> Dna
    ['GGCGTTCAGGCA', 'AAGAATCAGTCA', 'CAAGGAGTTCGC', 'CACGTCAATCAC', 'CAATAATATTCG']
    >>> k,t,Dna = read_file('test.gmswp.extra.txt')
    '''
    f = open(input_file)
